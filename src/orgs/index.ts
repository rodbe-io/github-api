...
export const getAllReposByOrg =
  (token: string) =>
  async <T = Repository>({ mapper, org }: GetAllReposByOrgProps<T>) => {
    const headers = {
...
export const getAllReposByOrgs = async <T = Repository, const Org extends string = string>({
  ...
  for (const org of orgs) {
    const res = await getAllReposByOrg(token)({ mapper, org });
...
export const getOrgs = <T>(token: string): T => {
  console.log('getOrgs test', token);
